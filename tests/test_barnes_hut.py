...
def _random_cloud(n, seed=0):
    ...
    return rng.normal(size=(n, 2)), rng.uniform(0.5, 1.5, n)


def test_theta_zero_is_exact():
    """With theta = 0 every cell is opened, so Barnes-Hut == direct sum."""
    pos, m = _random_cloud(120, seed=1)
...
def test_force_error_bounded_and_grows_with_theta():
    pos, m = _random_cloud(300, seed=2)
    direct = forces.accelerations(pos, m, G=1.0, softening=0.05)
...
def test_barnes_hut_conserves_energy_on_cluster():
    """A softened cluster integrated with Barnes-Hut keeps energy bounded."""
    sys = ic.plummer_sphere(n=150, seed=5)
